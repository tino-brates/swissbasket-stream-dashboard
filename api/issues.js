...
const AGENT_TOKEN_FOR_AUTH = "Auth0 3.3.0";
...
async function obtainTokenFromCookie() {
  ...
  const body = JSON.stringify({ agent: AGENT_TOKEN_FOR_AUTH, platform: "web" });
  const resp = await fetchJSON(`${BASE}/auth/token`, { method: "PUT", headers, body });
  debug.tried.push({ step: `PUT /auth/token [${AGENT_TOKEN_FOR_AUTH}]`, status: resp.status, raw: resp.raw?.slice(0, 200) });
  const token = resp.json?.access_token || resp.json?.accessToken || resp.json?.token || resp.json?.jwt || (resp.headers?.get("Authorization") || "").replace(/^OAuth2\s+/i, "").replace(/^Bearer\s+/i, "");
  if (token) { debug.tokenFound = true; debug.tokenSource = AGENT_TOKEN_FOR_AUTH; return { token, debug }; }
  const me = await fetchJSON(`${BASE}/me`, { method: "GET", headers: baseHeaders({ Cookie: `t=${COOKIE_T}` }) });
...
function isCriticalText(s) {
  ...
  return t.includes("offline") || t.includes("no ingest") || t.includes("encoder offline") || t.includes("unstable") || (t.includes("bandwidth") && (t.includes("low") || t.includes("insufficient") || t.includes("insuffisant")));
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ items: [], error: String(e), debug: { ...e } });
  }
}